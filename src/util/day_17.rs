...
fn water_path(map:&mut HashMap<(i32,i32),char>) {
    let y_max = *map.iter().map(|((x,y),ch)|y).max().unwrap();
    let start = (500,1);
...
fn parse(input:&str) -> HashMap<(i32,i32),char> {
    ...
    input.lines().for_each(|line|{
        ...
        for col in cols {
            //x=495 or y=2..7
            let variable_name = col.get(0..1).unwrap();
    ...
    let min_x = *pair_list.iter().map(|(x,y)|x.first().unwrap()).min().unwrap();
    let max_x = *pair_list.iter().map(|(x,y)|x.last().unwrap()).max().unwrap();
    let min_y = 0; //pair_list.iter().map(|(x,y)|y.first().unwrap()).min().unwrap();
    let max_y = *pair_list.iter().map(|(x,y)|y.last().unwrap()).max().unwrap();
...
fn print_map(map:&HashMap<(i32,i32),char>) {
    let max_x = *map.iter().map(|((x,y),ch)|x).max().unwrap();
    let min_x = *map.iter().map(|((x,y),ch)|x).min().unwrap();
    let max_y = *map.iter().map(|((x,y),ch)|y).max().unwrap();
    let min_y = *map.iter().map(|((x,y),ch)|y).min().unwrap();
...
mod tests {
    ...
    const INPUT_REAL:&str = "y=1160, x=336..349
y=1111, x=479..483
y=1369, x=409..419
x=413, y=698..701
x=450, y=953..962
